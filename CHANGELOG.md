...
### ✨ 새로운 기능
- **주식 주문 API 완전 구현**: 현금/신용 주문 및 주문가능조회 기능 추가
  - `order_stock_cash()`: 현금 매수/매도 주문 (지정가, 시장가, 최유리지정가 등)
  - `order_stock_credit()`: 신용 매수/매도 주문 (자기융자, 유통융자, 대주 등, 실전만)
  - `inquire_order_psbl()`: 매수 가능 수량/금액 조회
  - `inquire_credit_order_psbl()`: 신용매수 가능 수량/금액 조회

### 🔧 기술적 개선  
- **TR ID 업데이트**: 최신 한투 OpenAPI 규격 적용
  - 현금주문: 매수 TTTC0012U, 매도 TTTC0011U (모의: VTTC0012U, VTTC0011U)
  - 신용주문: 매수 TTTC0052U, 매도 TTTC0051U
- **Rate Limiter 최적화**: 10 req/sec, 100ms 간격으로 보수적 설정
- **포괄적 타입 힌트**: 모든 새 메서드에 완전한 타입 어노테이션
- **상세한 Docstring**: NumPy 스타일 문서화 및 실제 사용 예시
- **토큰 로깅 개선**: 불필요한 토큰 만료 메시지 제거

### 🧪 테스트 강화
- **246개 테스트 통과**: 기존 대비 68개 테스트 추가
- **52% 코드 커버리지**: 8% 향상
- **포괄적 단위 테스트**: 
  - StockAPI 주문 메서드 테스트 (13개 케이스)
  - Agent 래퍼 메서드 테스트 (9개 케이스)
  - 실전/모의 환경 분리 테스트
  - 매개변수 검증 및 에러 처리 테스트

### 📝 문서화 업데이트
- **README.md**: 새로운 주문 API 사용 예시 추가
- **실시간 주문 예시**: 삼성전자, 삼화전기 등 실제 종목 사용
- **최유리지정가 추천**: 빠른 체결을 위한 모범 사례

### 🔍 실전 검증
- **라이브 트레이딩 테스트**: 실제 주문 체결 검증 완료
- **틱 사이즈 준수**: 한국 주식시장 호가단위 정확한 적용
- **신용거래 지원**: 자기융자, 유통융자 등 모든 신용유형 지원
