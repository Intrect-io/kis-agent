...
class TestCacheIntegration:
    ...
    def test_api_cache_ttl_by_endpoint(self):
        ...
        # 시세 데이터는 10초 TTL
        price_ttl = cache.get_ttl_for_endpoint("/uapi/domestic-stock/v1/quotations/inquire-price")
        assert price_ttl == 10, f"시세 데이터 TTL은 10초여야 합니다. 실제: {price_ttl}"
        
        # 체결 데이터는 5초 TTL
        ccnl_ttl = cache.get_ttl_for_endpoint("/uapi/domestic-stock/v1/quotations/inquire-ccnl")
        assert ccnl_ttl == 5, f"체결 데이터 TTL은 5초여야 합니다. 실제: {ccnl_ttl}"
        
        # 일봉 데이터는 60초 TTL
        daily_ttl = cache.get_ttl_for_endpoint("/uapi/domestic-stock/v1/quotations/inquire-daily-itemchartprice")
        assert daily_ttl == 60, f"일봉 데이터 TTL은 60초여야 합니다. 실제: {daily_ttl}"
        
        # 종목 정보는 300초 TTL
        info_ttl = cache.get_ttl_for_endpoint("/uapi/domestic-stock/v1/quotations/inquire-stock-info")
        assert info_ttl == 300, f"종목 정보 TTL은 300초여야 합니다. 실제: {info_ttl}"
        
        # 주문은 캐시하지 않음 (0초)
        order_ttl = cache.get_ttl_for_endpoint("/uapi/domestic-stock/v1/trading/order-cash")
    ...
    def test_base_api_cache_integration(self, mock_request_manager):
        ...
        # Mock 클라이언트 설정
        mock_client = Mock()
        mock_client.make_request = Mock(return_value={
            "rt_cd": "0",
            "msg1": "성공",
            "output": [{"price": 70000}]
        })
...
class TestCacheTTLVerification:
    ...
    def test_order_no_cache(self):
        ...
        for endpoint in order_endpoints:
            ttl = cache.get_ttl_for_endpoint(endpoint)
            assert ttl == 0, f"주문 API는 캐시하지 않아야 합니다 (TTL=0): {endpoint}, 실제 TTL: {ttl}"
    ...
    def test_default_ttl_for_unknown_endpoint(self):
        ...
        assert ttl == 5, f"알 수 없는 엔드포인트는 기본 TTL을 사용해야 합니다. 실제: {ttl}"
