...
class TestAgentInit:
    ...
    def test_init_with_all_params(self):
        ...
        with patch.object(agent_module, "KISConfig") as mock_config:
            with patch.object(agent_module, "KISClient") as mock_client_class:
                mock_client = MockKISClient()
                mock_client_class.return_value = mock_client

                with patch.object(agent_module, "read_token") as mock_read_token:
                    mock_read_token.return_value = {"access_token": "valid_token"}

                    with patch.object(agent_module, "AccountAPI"):
                        with patch.object(agent_module, "StockAPI"):
                            with patch.object(agent_module, "StockInvestorAPI"):
                                with patch.object(agent_module, "ProgramTradeAPI"):
                                    with patch.object(agent_module, "StockMarketAPI"):
                                        with patch.object(
                                            agent_module, "InterestStockAPI"
                                        ):
                                            ...
                                            assert (
                                                agent.account_info["CANO"] == "12345678"
                                            )
                                            assert (
                                                agent.account_info["ACNT_PRDT_CD"]
                                                == "01"
                                            )
            ...
            with patch.object(agent_module, "AccountAPI"):
                with patch.object(agent_module, "StockAPI"):
                    with patch.object(agent_module, "StockInvestorAPI"):
                        with patch.object(agent_module, "ProgramTradeAPI"):
                            with patch.object(agent_module, "StockMarketAPI"):
                                with patch.object(agent_module, "InterestStockAPI"):
                                    agent = agent_module.Agent(
                                        app_key="KEY",
                                        app_secret="SECRET",
                                        account_no="12345",
    ...
    def test_init_with_rate_limiter_config(self):
        ...
        with patch.object(agent_module, "read_token") as mock_read_token:
            ...
            with patch.object(
                agent_module, "get_global_rate_limiter"
            ) as mock_get_limiter:
...
class TestEnsureValidToken:
    """_ensure_valid_token 테스트 (L232-257)"""

    def test_token_not_found_new_token_issued(self, caplog):
        """토큰이 없을 때 새로 발급 (L243-249)"""
        agent_module = get_agent_module()

        mock_client = MockKISClient()

        with patch.object(agent_module, "read_token") as mock_read_token:
            # 첫 번째 호출: None 반환 (토큰 없음)
            mock_read_token.return_value = None

            with patch.object(agent_module, "auth") as mock_auth:
    ...
    def test_valid_token_found(self, caplog):
        """유효한 토큰이 있을 때 재사용 (L250-252)"""
        agent_module = get_agent_module()

        mock_client = MockKISClient()

        with patch.object(agent_module, "read_token") as mock_read_token:
            mock_read_token.return_value = {"access_token": "existing_token"}
                            ...
                            with patch.object(agent_module, "ProgramTradeAPI"):
                                with patch.object(agent_module, "StockMarketAPI"):
                                    with patch.object(agent_module, "InterestStockAPI"):
                                        ...
                                        # 토큰이 유효하면 auth 호출 안 함
                                        mock_auth.assert_not_called()

    def test_token_validation_error_raises_runtime_error(self):
        """토큰 검증/발급 실패 시 RuntimeError 발생 (L254-257)"""
        agent_module = get_agent_module()

        mock_client = MockKISClient()

        with patch.object(agent_module, "read_token") as mock_read_token:
            mock_read_token.side_effect = Exception("Token read failed")
            ...
            assert "토큰 자동 발급 실패" in str(exc_info.value)
...
class TestWebsocket:
    ...
    def test_websocket_creates_kis_websocket(self):
        ...
        with patch.object(agent_module, "read_token") as mock_read_token:
            mock_read_token.return_value = {"access_token": "token"}
                        ...
                        with patch.object(agent_module, "ProgramTradeAPI"):
                            with patch.object(agent_module, "StockMarketAPI"):
                                with patch.object(agent_module, "InterestStockAPI"):
                                    ...
                                    # ConditionAPI를 모의
                                    with patch(
                                        "pykis.stock.condition.ConditionAPI"
                                    ) as mock_condition:
                                        mock_condition_instance = MagicMock()
                                        mock_condition_instance.get_condition_stocks.return_value = (
                                            mock_result
                                        )
                                        mock_condition.return_value = (
                                            mock_condition_instance
                                        )
                            ...
                            with patch.object(agent_module, "StockMarketAPI"):
                                with patch.object(agent_module, "InterestStockAPI"):
                                    ...
                                    with patch(
                                        ...
                                        mock_condition.side_effect = Exception(
                                            "API 오류"
                                        )
...
class TestGetTopGainers:
    ...
    def test_get_top_gainers_success(self):
        ...
        with patch.object(agent_module, "read_token") as mock_read_token:
            ...
            with patch.object(agent_module, "AccountAPI"):
                ...
                with patch.object(agent_module, "StockAPI") as mock_stock_class:
                    mock_stock_class.return_value = mock_stock_api
    ...
    def test_get_top_gainers_failure(self, caplog):
        ...
        with patch.object(agent_module, "read_token") as mock_read_token:
            ...
            with patch.object(agent_module, "AccountAPI"):
                mock_stock_api = MagicMock()
                mock_stock_api.get_market_fluctuation.side_effect = Exception(
                    "API 오류"
                )
...
class TestOrderStockCash:
    ...
    def test_order_stock_cash_buy(self):
        ...
        with patch.object(agent_module, "read_token") as mock_read_token:
            ...
            with patch.object(agent_module, "AccountAPI") as mock_account_class:
                mock_account_class.return_value = mock_account_api
                            ...
                            with patch.object(agent_module, "StockMarketAPI"):
                                with patch.object(agent_module, "InterestStockAPI"):
                                    agent = agent_module.Agent(
                                        ...
                                        account_code="01",
                                        client=mock_client,
                                        enable_rate_limiter=mock_rate_limiter
                                        is not None,
                                        rate_limiter=mock_rate_limiter,
                                    )
                                    return agent
...
class TestGetAttr:
    ...
    def test_getattr_raises_attribute_error(self):
        ...
        with patch.object(agent_module, "read_token") as mock_read_token:
            ...
            mock_account_api = MagicMock(spec=[])
            mock_stock_api = MagicMock(spec=[])
            mock_investor_api = MagicMock(spec=[])
            mock_program_api = MagicMock(spec=[])
            mock_market_api = MagicMock(spec=[])
            mock_interest_api = MagicMock(spec=[])

            with patch.object(agent_module, "AccountAPI") as mock_acc_class:
                mock_acc_class.return_value = mock_account_api
                with patch.object(agent_module, "StockAPI") as mock_stk_class:
                    mock_stk_class.return_value = mock_stock_api
                    with patch.object(
                        agent_module, "StockInvestorAPI"
                    ) as mock_inv_class:
                        mock_inv_class.return_value = mock_investor_api
                        with patch.object(
                            agent_module, "ProgramTradeAPI"
                        ) as mock_prg_class:
                            mock_prg_class.return_value = mock_program_api
                            with patch.object(
                                agent_module, "StockMarketAPI"
                            ) as mock_mkt_class:
                                mock_mkt_class.return_value = mock_market_api
                                with patch.object(
                                    agent_module, "InterestStockAPI"
                                ) as mock_int_class:
                                    mock_int_class.return_value = mock_interest_api
