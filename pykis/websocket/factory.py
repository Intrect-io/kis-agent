    ...
    def create_client(
        ...
        **kwargs
        ...
        if client_type == ClientType.BASIC:
            ...
            return WebSocketClientFactory._create_realtime_client(approval_key, **kwargs)
        elif client_type == ClientType.MONITORING:
            return WebSocketClientFactory._create_monitoring_client(approval_key, **kwargs)
        elif client_type == ClientType.BACKTEST:
            return WebSocketClientFactory._create_backtest_client(approval_key, **kwargs)
        else:
            raise ValueError(f"지원하지 않는 클라이언트 타입: {client_type}")
            
    @staticmethod
    def _create_basic_client(
        approval_key: str,
        **kwargs
    ) -> RefactoredWebSocketClient:
        """
