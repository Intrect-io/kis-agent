...
class TradeHandler(MessageHandler):
    ...
    def can_handle(self, message: Dict[str, Any]) -> bool:
        """체결 메시지 확인"""
        return message.get('tr_id') == self.tr_id or \
               message.get('header', {}).get('tr_id') == self.tr_id
...
class PingPongHandler(MessageHandler):
    ...
    def can_handle(self, message: Dict[str, Any]) -> bool:
        """PINGPONG 메시지 확인"""
        return message.get('type') == 'PINGPONG' or \
               message.get('header', {}).get('tr_id') == 'PINGPONG'
