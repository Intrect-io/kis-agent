...
class TradingReportGenerator:
        ...
        try:
            ...
            if only_executed:
                ...
                if 'tot_ccld_qty' in df.columns:
                    ...
                    df['tot_ccld_qty'] = pd.to_numeric(df['tot_ccld_qty'], errors='coerce').fillna(0)
                    df = df[df['tot_ccld_qty'] > 0]
            
            # 데이터 타입 정리
            numeric_columns = ['ord_qty', 'ord_unpr', 'tot_ccld_qty', 'avg_prvs', 'ccld_amt']
            ...
            if 'ord_dt' in df.columns:
                df['ord_dt'] = pd.to_datetime(df['ord_dt'], format='%Y%m%d', errors='coerce')
    ...
    def export_to_excel(
        ...
        try:
            ...
            if output_path is None:
                ...
                output_path = f"trading_history_{start_date}_{end_date}_{timestamp}.xlsx"
            ...
            with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
                ...
                if tickers and separate_sheets:
                    ...
                    for ticker in tickers:
                        ...
                        if not df.empty:
                            # 종목명 가져오기
                            ticker_name = df['prdt_name'].iloc[0] if 'prdt_name' in df.columns else ticker
                            sheet_name = f"{ticker}_{ticker_name[:10]}"  # 시트명 길이 제한
    ...
    def _write_sheet(self, writer: pd.ExcelWriter, df: pd.DataFrame, sheet_name: str):
        ...
        if df.empty:
            ...
            df = pd.DataFrame(columns=[
                '주문일자', '주문시각', '종목코드', '종목명', '매매구분',
                '주문수량', '주문단가', '체결수량', '체결단가', '체결금액'
            ])
        else:
            # 컬럼명 한글화
            column_mapping = {
    ...
    def _add_summary_sheet(self, writer: pd.ExcelWriter, start_date: str, end_date: str):
        """요약 시트 추가
