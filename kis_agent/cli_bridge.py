...
def _parse_request(line: str) -> Dict[str, Any]:
    ...
    if "method" not in request:
        raise KeyError("Request missing required field: 'method'")

    method = request.get("method")
    if not method or not isinstance(method, str):
        raise ValueError("Request must have 'method' field (non-empty string)")
...
def main() -> None:
    ...
    try:
        ...
        error_response = BridgeResponse.error(str(e), "KeyError")
        print(error_response, file=sys.stdout)
        sys.exit(1)
    except ValueError as e:
        # Invalid request format or value
