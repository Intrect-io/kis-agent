...
def print_orderbook_analysis(orderbook_data, title="호가창 분석"):
    ...
    if not orderbook_data or orderbook_data.get('rt_cd') != '0':
        print(f"조회 실패: {orderbook_data.get('msg1', '알 수 없는 오류') if orderbook_data else '응답 없음'}")
        return
    ...
    print(f"전일대비: {output1.get('prdy_vrss', 'N/A')} ({output1.get('prdy_ctrt', 'N/A')}%)")
    print(f"미결제약정: {output1.get('optn_opni_qty', 'N/A')} 계약")
    print(f"미결제약정 전일대비: {output1.get('optn_opni_prdy_dfrn', 'N/A')}")

    # 호가창 10단계 출력
    print(f"\n[호가창 - 10단계]")
    print(f"{'단계':<4} {'매도잔량':>12} {'매도호가':>12} | {'매수호가':>12} {'매수잔량':>12}")
    print("-" * 70)
    ...
    for i in range(1, 11):
        ...
        print(f"{i:>2}단  {ask_volume:>12} {ask_price:>12} | {bid_price:>12} {bid_volume:>12}")
...
def main():
    ...
    stock_futures_orderbook = agent.get_future_orderbook(
        code=stock_futures_code,
        market_div_code="JF"  # 주식선물
    )
